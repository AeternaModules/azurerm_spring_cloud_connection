...
output "spring_cloud_connections_authentication" {
  ...
  value       = { for k, v in azurerm_spring_cloud_connection.spring_cloud_connections : k => one(v.authentication) if v.authentication != null && length(v.authentication) > 0 }
  sensitive   = true
}
...
output "spring_cloud_connections_secret_store" {
  ...
  value       = { for k, v in azurerm_spring_cloud_connection.spring_cloud_connections : k => one(v.secret_store) if v.secret_store != null && length(v.secret_store) > 0 }
}
